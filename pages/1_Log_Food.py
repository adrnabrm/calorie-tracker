...
from services.calculations import scale_macros, to_grams
...
def log_today(food: dict, eaten_grams: float) -> None:
...
if not foods:
    ...
    labels = {
        f["id"]: f"{f['name']} ({float(f['serving_grams']):g} g serving)" for f in foods
    }
    ...
    lib_unit = st.segmented_control("Unit", ["g", "oz"], default="g", key="lib_unit")
    if st.button("Log"):
        food = next(f for f in foods if f["id"] == food_id)
        log_today(food, to_grams(lib_eaten, lib_unit or "g"))
        st.success("Logged.")
...
if submitted:
    serving_grams = to_grams(serving_size, serving_unit or "g")
    row = insert_food(
        Food(
            name=name,
            calories=calories,
    ...
    log_today(row, to_grams(new_eaten, new_unit or "g"))
